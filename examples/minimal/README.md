...
The smallest valid call to this module: one Linux flexible scale set on a fresh vnet, one
instance, one NIC, SSH-only auth with a placeholder public key (nothing ever logs in), a catalog
image, and the module defaults. The environment comes from the Terraform workspace
...
| Name | Description |
|------|-------------|
| <a name="output_image_catalog_keys"></a> [image\_catalog\_keys](#output\_image\_catalog\_keys) | Catalog keys accepted by source\_image\_simple. |
